...
export default function WorldMapColombia({ active }) {
  ...
  const shapes = useMemo(() => {
    if (!colombia) return []
    const { type, coordinates } = colombia.geometry
    const coordsList =
      type === 'Polygon'
        ? coordinates[0]
        : coordinates[0][0]
  ...
  const countryData = {
    name: 'Colombia',
    legal: 'Legal',
    details: '15-12-2014, Ruling of the Constitutional Court of Colombia'
  }
