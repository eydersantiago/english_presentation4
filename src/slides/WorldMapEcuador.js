...
export default function WorldMapEcuador({ active }) {
  const [hovered, setHovered] = useState(false)
  const [tooltipPos, setTooltipPos] = useState([0, 0, 0])

  // 2) Feature de Colombia
  const ecuador = useMemo(
    () => geoData.features.find(f => f.properties.iso_a3 === 'ECU'),
    []
  )
  ...
  const shapes = useMemo(() => {
    if (!ecuador) return []
    const { type, coordinates } = ecuador.geometry
    const coordsList =
      type === 'Polygon'
        ? coordinates[0]
        : coordinates[0][0]
  ...
  const countryData = {
    name: 'Ecuador',
    legal: 'Legal',
    details: '05-02-2024 Ruling of the Constitutional Court of Ecuador'
  }
